' FREQPLAN - Doppler tuning schedule for a pass (PicoCalc / MMBasic)
' Per-step downlink/uplink dial frequencies, inverting transponder
' handled. Orbit model: secular-J2 mean elements + Kepler (shared core).
OPTION EXPLICIT
...
SUB Setup
  LOCAL g$, inc, ecc, ra, ap, ma, mm, stp
  LOCAL ey, emo, ed, eh, emi, es, ny, nmo, nd, nh, nmi
  LOCAL fdn, fup, inv, j, t, rr, ds, us, dn, up
  CLS
  PRINT "FREQPLAN - Doppler tuning plan"
  INPUT "Grid [FM18LV] "; g$
  IF g$ = "" THEN g$ = "FM18LV"
  CALL Maiden(g$)
  INPUT "INC  "; inc
  ...
  INPUT " Hr "; nh
  INPUT " Mi "; nmi
  INPUT "Downlink MHz [145.95] "; fdn
  IF fdn = 0 THEN fdn = 145.95
  INPUT "Uplink MHz [435.1] "; fup
  IF fup = 0 THEN fup = 435.1
  INPUT "Inverting? 1/0 [1] "; inv
  INPUT "Step sec [60] "; stp
  IF stp <= 0 THEN stp = 60
  ...
  IF PAOS = 0 THEN
    ...
    EXIT SUB
  ENDIF
  PRINT "MaxEl "; STR$(PMEL * RAD, 0, 0); " deg"
  PRINT "UTC    DnDial    UpDial"
  t = PAOS
  DO WHILE t <= PLOS + 1E-9
    rr = RangeRate(t)
    ds = -fdn * 1000000.0 * rr / CKMS
    us = -fup * 1000000.0 * rr / CKMS
    dn = fdn + ds / 1000000.0
    IF inv = 0 THEN up = fup + us / 1000000.0 ELSE up = fup - us / 1000000.0
    CALL Cal(t)
    PRINT FMT2$(CH); ":"; FMT2$(CMI); " "; STR$(dn, 0, 4); " "; STR$(up, 0, 4)
    t = t + stp / 86400.0
  LOOP
END SUB
