...
' WINDOW - horizon-mask-aware effective AOS/LOS (PicoCalc / MMBasic).
DIM ma0(20), ma1(20), mae(20)
DIM a0in, a1in, eein, step, t, eaos, elos, maxv, thr, az
DIM INTEGER nmask, mi
...
INPUT "  Hr "; nh
INPUT "  Mi "; nmi
PRINT "Mask sectors az0 az1 el (az0<0 ends)"
nmask = 0
DO
  INPUT "az0 "; a0in
  IF a0in < 0 THEN EXIT DO
  INPUT "az1 "; a1in
  INPUT "el  "; eein
  nmask = nmask + 1
  ma0(nmask) = a0in : ma1(nmask) = a1in : mae(nmask) = eein
LOOP
...
DO
  ...
  IF LEL >= 0 THEN
    ...
    thr = 0
    FOR mi = 1 TO nmask
      IF ma0(mi) <= ma1(mi) THEN
        IF az >= ma0(mi) AND az <= ma1(mi) AND mae(mi) > thr THEN thr = mae(mi)
      ELSE
        IF (az >= ma0(mi) OR az <= ma1(mi)) AND mae(mi) > thr THEN thr = mae(mi)
      ENDIF
    NEXT mi
    IF LEL * RAD >= thr THEN
      IF eaos = 0 THEN eaos = t
      elos = t
      IF LEL * RAD > maxv THEN maxv = LEL * RAD
    ENDIF
  ENDIF
  t = t + step
  IF t > PLOS + 0.000000001 THEN EXIT DO
LOOP
CALL Cal(PAOS)
PRINT "Geo AOS "; STR$(CH,2,0,"0"); ":"; STR$(CMI,2,0,"0");
CALL Cal(PLOS)
PRINT " LOS "; STR$(CH,2,0,"0"); ":"; STR$(CMI,2,0,"0"); " El"; STR$(PMEL * RAD,3,0)
IF eaos = 0 THEN PRINT "Behind mask whole pass." : END
CALL Cal(eaos)
PRINT "Eff AOS "; STR$(CH,2,0,"0"); ":"; STR$(CMI,2,0,"0");
CALL Cal(elos)
PRINT " LOS "; STR$(CH,2,0,"0"); ":"; STR$(CMI,2,0,"0"); " El"; STR$(maxv,3,0)
END
