...
SUB Setup
  ...
  LOCAL j, t, ss
  CLS
  PRINT "POINTING - next pass Az/El table"
  INPUT "Grid [FM18LV] "; g$
  IF g$ = "" THEN g$ = "FM18LV"
  CALL Maiden(g$)
  INPUT "INC  "; inc
  ...
  CALL Cal(PAOS)
  PRINT
  PRINT "AOS "; FMT2$(CH); ":"; FMT2$(CMI); "Z";
  CALL Cal(PLOS)
  PRINT "  LOS "; FMT2$(CH); ":"; FMT2$(CMI); "Z"
  ...
  DO WHILE t <= PLOS + 1E-9
    ...
    IF LEL < 0 THEN LEL = 0
    CALL Cal(t)
    ss = INT((t * 86400.0) - INT(t * 86400.0 / 60) * 60 + 0.5)
    PRINT FMT2$(CH); ":"; FMT2$(CMI); ":"; FMT2$(ss); " ";
    PRINT STR$(LAZ * RAD, 4, 0); " "; STR$(LEL * RAD, 4, 0); " "; STR$(LRNG, 5, 0)
    t = t + stp / 86400.0
  LOOP
END SUB
