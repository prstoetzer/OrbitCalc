...
' SKEDQSO - mutual-pass scheduler for two grids (PicoCalc / MMBasic).
DIM g1$, g2$, la1, lo1, la2, lo2
DIM inc, ecc, ra, ap, ma, mm
DIM ey, emo, ed, eh, emi, es
DIM ny, nmo, nd, nh, nmi
DIM days, minel, now, hor, endt, step, j, j2, mme, t, e1, e2, mn, wstart
DIM INTEGER cnt, nw, guard, inwin, nowup, kk
PRINT "SKEDQSO - mutual scheduler"
INPUT "Your grid [FM18LV] "; g1$
IF g1$ = "" THEN g1$ = "FM18LV"
INPUT "Their grid [CM87XX] "; g2$
IF g2$ = "" THEN g2$ = "CM87XX"
CALL Maiden(g1$) : la1 = OBLAT : lo1 = OBLON
CALL Maiden(g2$) : la2 = OBLAT : lo2 = OBLON
INPUT "INC  "; inc
INPUT "ECC  "; ecc
INPUT "RAAN "; ra
INPUT "ARGP "; ap
...
INPUT "  Hr "; nh
INPUT "  Mi "; nmi
INPUT "Days ahead "; days
INPUT "Min elev both "; minel
INPUT "How many "; cnt
...
endt = now + days
step = 30.0 / 86400.0
PRINT "Date  Start LOS  El"
j = now
CALL Look(j, la1, lo1) : e1 = LEL
CALL Look(j, la2, lo2) : e2 = LEL
inwin = 0
IF e1 >= hor AND e2 >= hor THEN inwin = 1
wstart = j
nw = 0
guard = 0
DO WHILE j < endt AND nw < cnt AND guard < 2000000
  guard = guard + 1
  j2 = j + step
  CALL Look(j2, la1, lo1) : e1 = LEL
  CALL Look(j2, la2, lo2) : e2 = LEL
  nowup = 0
  IF e1 >= hor AND e2 >= hor THEN nowup = 1
  IF nowup = 1 AND inwin = 0 THEN wstart = j2
  IF nowup = 0 AND inwin = 1 THEN
    mme = -90.0
    FOR kk = 0 TO 12
      t = wstart + (j2 - wstart) * kk / 12.0
      CALL Look(t, la1, lo1) : e1 = LEL
      CALL Look(t, la2, lo2) : e2 = LEL
      mn = e1
      IF e2 < e1 THEN mn = e2
      IF mn * RAD > mme THEN mme = mn * RAD
    NEXT kk
    CALL Cal(wstart)
    PRINT STR$(CD,2,0,"0"); "/"; STR$(CMO,2,0,"0"); " "; STR$(CH,2,0,"0"); ":"; STR$(CMI,2,0,"0"); " ";
    CALL Cal(j2)
    PRINT STR$(CH,2,0,"0"); ":"; STR$(CMI,2,0,"0"); " "; STR$(mme,3,0)
    nw = nw + 1
  ENDIF
  inwin = nowup
  j = j2
LOOP
IF nw = 0 THEN PRINT "(none)"
END
