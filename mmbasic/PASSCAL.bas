...
' PASSCAL - multi-day pass calendar with a min max-elevation filter.
' PicoCalc / MMBasic. Lists workable passes over N days.
DIM g$, inc, ecc, ra, ap, ma, mm
DIM ey, emo, ed, eh, emi, es
DIM ny, nmo, nd, nh, nmi
DIM days, minel, now, endt, t, az, dur
DIM INTEGER np, guard
PRINT "PASSCAL - pass calendar"
INPUT "Grid [FM18LV] "; g$
IF g$ = "" THEN g$ = "FM18LV"
CALL Maiden(g$)
INPUT "INC  "; inc
...
t = now
guard = 0
DO WHILE t < endt AND guard < 400
  guard = guard + 1
  CALL FindPass(t)
  IF PAOS = 0 THEN EXIT DO
  IF PAOS >= endt THEN EXIT DO
  IF PMEL * RAD >= minel THEN
    CALL Look(PTCA, OBLAT, OBLON)
    az = LAZ * RAD
    CALL Cal(PAOS)
    PRINT STR$(CD,2,0,"0"); "/"; STR$(CMO,2,0,"0"); " "; STR$(CH,2,0,"0"); ":"; STR$(CMI,2,0,"0"); " ";
    CALL Cal(PLOS)
    PRINT STR$(CH,2,0,"0"); ":"; STR$(CMI,2,0,"0"); " ";
    PRINT STR$(PMEL * RAD,3,0); " "; STR$(az,3,0)
    np = np + 1
  ENDIF
  t = PLOS + 60.0 / 86400.0
LOOP
IF np = 0 THEN PRINT "(none)"
END
