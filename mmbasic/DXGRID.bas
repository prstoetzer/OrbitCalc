...
' DXGRID - Maidenhead fields inside the footprint (PicoCalc / MMBasic).
DIM seen$(200)
DIM now, slat, slon, rho, r, lat, lon, d, x, y
DIM f$, line$
DIM INTEGER ns, dup, ii
PRINT "DXGRID - footprint grids"
...
CALL Ecef(now)
slat = ATAN2S(EZ, SQR(EX * EX + EY * EY)) * RAD
slon = ATAN2S(EY, EX) * RAD
CALL Eci(now)
r = SQR(GX * GX + GY * GY + GZ * GZ)
rho = ACOSS(ERAD / r) * RAD
PRINT "Sub "; STR$(slat,0,2); ","; STR$(slon,0,2); " foot "; STR$(rho,0,1)
...
DO WHILE lat <= 88
  lon = -178
  DO WHILE lon <= 178
    d = GCDEG(slat, slon, lat, lon)
    IF d <= rho THEN
      x = lon + 180 : y = lat + 90
      f$ = CHR$(65 + INT(x / 20)) + CHR$(65 + INT(y / 10))
      dup = 0
      FOR ii = 1 TO ns
        IF seen$(ii) = f$ THEN dup = 1
      NEXT ii
      IF dup = 0 THEN ns = ns + 1 : seen$(ns) = f$
    ENDIF
    lon = lon + 4
  LOOP
  lat = lat + 4
...
FOR ii = 1 TO ns
  line$ = line$ + seen$(ii) + " "
  IF ii MOD 10 = 0 THEN PRINT line$ : line$ = ""
NEXT ii
IF line$ <> "" THEN PRINT line$
PRINT "("; STR$(ns,0,0); " fields)"
END

FUNCTION GCDEG(la1, lo1, la2, lo2)
  LOCAL a, b, dl, c
  a = la1 * DEG : b = la2 * DEG : dl = (lo2 - lo1) * DEG
  c = SIN(a) * SIN(b) + COS(a) * COS(b) * COS(dl)
  IF c > 1 THEN c = 1
  IF c < -1 THEN c = -1
  GCDEG = ACOSS(c) * RAD
END FUNCTION
