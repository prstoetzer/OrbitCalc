...
' ROTOR - pass Az/El table with optional flip-mode (PicoCalc / MMBasic).
DIM stp, t, az, el, ad, ed2
DIM INTEGER flp, ss
PRINT "ROTOR - pass Az/El"
...
DO
  ...
  IF flp = 1 THEN
    ad = ad + 180.0
    IF ad >= 360.0 THEN ad = ad - 360.0
    ed2 = 180.0 - ed2
  ENDIF
  CALL Cal(t)
  ss = INT(CS + 0.5) : IF ss >= 60 THEN ss = 0
  PRINT STR$(CH,2,0,"0"); ":"; STR$(CMI,2,0,"0"); ":"; STR$(ss,2,0,"0"); " "; STR$(ad,4,0); " "; STR$(ed2,3,0)
  t = t + stp / 86400.0
  IF t > PLOS + 0.000000001 THEN EXIT DO
LOOP
IF flp = 1 THEN PRINT "(flip: az+180 el=180-el)"
END
