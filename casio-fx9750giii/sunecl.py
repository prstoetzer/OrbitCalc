...
def draw(now, toff):
    ...
    sel, saz = sun_look(j)
    mo, d, h, mi = cal(j)
    draw_string(0, 0, "SUN/ECL %02d:%02dZ" % (h, mi), BLACK, "medium")
    daystr = "day" if sel >= 0 else ("twi" if sel > -6 * DEG else "night")
    draw_string(0, 11, "Sun El%d Az%d %s" % (int(sel * RAD), int(saz * RAD), daystr), BLACK, "medium")
    lit = is_lit(j)
    draw_string(0, 23, "Sat: " + ("SUNLIT" if lit else "ECLIPSE"), BLACK, "medium")
    # timeline next 100 min
    draw_string(0, 35, "Next 100min:", BLACK, "medium")
    y0 = 45
    y1 = 53
    ...
    if tnext is not None:
        ...
        if cur:
            draw_string(0, 56, "->eclipse %02d:%02d" % (h2, m2), BLACK, "medium")
        else:
            draw_string(0, 56, "->sunlit %02d:%02d" % (h2, m2), BLACK, "medium")
    show_screen()
...
def main():
    ...
    # re-run with a different time offset to step).
