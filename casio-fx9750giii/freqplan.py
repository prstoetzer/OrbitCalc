# freqplan.py - Doppler tuning schedule for a pass, Casio fx-9750GIII
# Python mode. Per-step downlink/uplink dial frequencies across a pass
# (inverting-transponder uplink handled). Self-contained secular-J2
# propagator. MicroPython 1.9.4 safe dialect.
import math
...
def range_rate(j, la, lo):
    dt = 1.0 / 86400.0
    e0, a0, r0 = look(j - dt, la, lo)
    e1, a1, r1 = look(j + dt, la, lo)
    return (r1 - r0) / 2.0


def main():
    print("FreqPlan (fx-9750GIII)")
    ...
    step = ask("Step sec", 60)
    loadsat()
    now = jd(NW[0], NW[1], NW[2], NW[3], NW[4], 0)
    pr = find_pass(now, la, lo)
    if pr is None:
        print("No pass in 14 days.")
        return
    aos, tca, los, mel = pr
    print("MaxEl %.0f" % (mel * RAD))
    print("UTC   DnDial   UpDial")
    t = aos
    while t <= los + 0.000000001:
        rr = range_rate(t, la, lo)
        ...
        if inv:
            ...
            up = fup + us / 1000000.0
        zz, z4, hh, mm2 = cal(t)
        print("%02d:%02d %8.4f %8.4f" % (hh, mm2, dn, up))
        t = t + step / 86400.0
