...
def both(j, la1, lo1, la2, lo2, hor):
    e1, a1, r1 = look(j, la1, lo1)
    e2, a2, r2 = look(j, la2, lo2)
    if e1 >= hor and e2 >= hor:
        return 1
    return 0


def main():
    print("SkedQSO (fx-9750GIII)")
    ...
    step = 30.0 / 86400.0
    print("Mutual >=" + str(minel) + " deg")
    print("Date  Start LOS  El")
    j = now
    inwin = both(j, la1, lo1, la2, lo2, hor)
    wstart = j
    nw = 0
    guard = 0
    while j < end and nw < cnt and guard < 2000000:
        guard = guard + 1
        j2 = j + step
        nowup = both(j2, la1, lo1, la2, lo2, hor)
        if nowup == 1 and inwin == 0:
            wstart = j2
        if nowup == 0 and inwin == 1:
            mme = -90.0
            for k in range(13):
                t = wstart + (j2 - wstart) * k / 12.0
                e1, a1, r1 = look(t, la1, lo1)
                e2, a2, r2 = look(t, la2, lo2)
                mn = e1
                if e2 < e1:
                    mn = e2
                if mn * RAD > mme:
                    mme = mn * RAD
            mo, d, h, mi = cal(wstart)
            z, z2, h2, m2 = cal(j2)
            print("%02d/%02d %02d:%02d %02d:%02d %3.0f" % (d, mo, h, mi, h2, m2, mme))
            nw = nw + 1
        inwin = nowup
        j = j2
    if nw == 0:
