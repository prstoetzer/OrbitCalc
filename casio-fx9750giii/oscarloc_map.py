# OSCARLOC - Live polar OSCARLOCATOR for the Casio fx-9750GIII
...
# live sub-satellite point stepping in time, a range circle
# over your QTH, and a text read-out.
#
# Orbit model: secular-J2 mean-element propagation + Kepler.
# Reference geometry for planning, not precise pointing.
#
# Controls (press a key, then EXE if your OS buffers input):
#   Use the menu prompts. After the map is up:
#     +  / 6  : step forward      -  / 4 : step back
#     F       : bigger step       S      : smaller step
#     H       : flip hemisphere   Q / EXIT : quit
# On the calculator, getkey() returns key codes; see KEYMAP below.

from casioplot import set_pixel, draw_string, show_screen, clear_screen
import math

try:
    from casioplot import getkey      # available on newer OS
except ImportError:
    getkey = None
...
def draw_frame(now, toff):
    ...
    mo, d, h, mi = cal(j)
    draw_string(72, 2, z2(h) + ":" + z2(mi) + "Z", BLACK, "small")
    el, az, rng = look(j)
    sub = "%d%s %d%s" % (abs(int(la * RAD)), "N" if la >= 0 else "S",
                         abs(int(lo * RAD)), "E" if lo >= 0 else "W")
    draw_string(72, 14, sub, BLACK, "small")
    if el >= 0:
        draw_string(72, 26, "Az%d" % int(az * RAD), BLACK, "small")
        draw_string(72, 38, "El%d" % int(el * RAD), BLACK, "small")
    else:
        draw_string(72, 26, "below", BLACK, "small")
        draw_string(72, 38, "horiz", BLACK, "small")
    draw_string(0, 56, SATNM + " T+" + str(int(toff)) + "m", BLACK, "small")
    show_screen()


# ---- key handling ----
# fx-9750GIII getkey() codes (subset). If getkey is missing we fall
# back to input() so the program is still usable / testable.
def wait_step():
    # returns one of: 'fwd','back','faster','slower','hemi','quit'
    if getkey is not None:
        while True:
            k = getkey()
            if k in (74, 42):       # + or 6/right-ish
                return 'fwd'
            if k in (75, 41):       # - or 4/left-ish
                return 'back'
            if k == 73:             # 'F' area -> use multiply? mapped loosely
                return 'faster'
            if k == 72:
                return 'slower'
            if k == 78:             # H
                return 'hemi'
            if k in (47, 76):       # EXIT / AC
                return 'quit'
    else:
        s = input("[+]fwd [-]back [F]aster [S]lower [H]emi [Q]uit: ").strip().upper()
        if s in ("+", "6", "F W D", "FWD", ""):
            return 'fwd'
        if s in ("-", "4", "BACK"):
            return 'back'
