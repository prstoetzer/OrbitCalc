...
def main():
    ...
    nf = 0
    guard = 0
    while nf < cnt and guard < 500000:
        guard = guard + 1
        j2 = j + step
        lat2, lon2 = subpt(j2)
        cr = 0
        if asc == 1 and plat < 0 and lat2 >= 0:
            cr = 1
        if asc == 0 and plat > 0 and lat2 <= 0:
            cr = 1
        if cr == 1:
            a = j
            b = j2
            ...
            if v < 0:
                ew = "W"
                v = -v
            print("%02d/%02d %02d:%02d %6.2f%s" % (d, mo, h, mi, v, ew))
            nf = nf + 1
        plat = lat2
        j = j2
