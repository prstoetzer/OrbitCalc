...
def draw(now):
    ...
    clear_screen()
    if pr is None:
        draw_string(0, 24, "No pass in 14 days", BLACK, "medium")
        show_screen()
        return
    aos, tca, los = pr
    # plot box
    ...
    for e in (30, 60, 90):
        ...
        draw_string(0, yy - 3, str(e), BLACK, "medium")
    last = None
    me = 0
    ...
    mo, d, h, mi = cal(aos)
    draw_string(0, 50, "AOS %02d:%02d" % (h, mi), BLACK, "medium")
    mo, d, h2, m2 = cal(los)
    draw_string(64, 50, "LOS %02d:%02d" % (h2, m2), BLACK, "medium")
    draw_string(0, 58, "MaxEl %d  Dur %dm" % (int(me * RAD), round((los - aos) * 1440)), BLACK, "medium")
    show_screen()
...
def main():
    ...
    skip = ask("Skip ahead min", 0)
    loadsat()
    now = jd(NW[0], NW[1], NW[2], NW[3], NW[4], 0) + skip / 1440.0
    draw(now)
    # Graph persists until EXIT; re-run with a larger 'Skip ahead'
    # to see a later pass.
